...
public class Pharmacy implements Serializable, DatabaseOperations {
	...
	public Map<String, Integer> getProductLocationForRequiredMedicine(Medicine chosenMedicine, int stockNeeded)
		...
		if (availableInStock(chosenMedicine, stockNeeded)) {
			for (Drawer drawer : drawers) {
				if (drawer.brandExistsInDrawer(chosenMedicine.getBrand())
						&& drawer.barcodeExistsInDrawer(chosenMedicine.getBarcode())) {
	...
	public Map<String, Integer> getRequestedQuantity(int stockNeeded, Map<String, Integer> drawersWithChosenMedicine,
			Medicine chosenMedicine) throws InsufficientStockException {
		...
		if (availableInStock(chosenMedicine, stockNeeded)) {
			for (String drawerName : drawersWithChosenMedicine.keySet()) {
				...
				if (stock > stockNeeded) {
					if (previous == 0) {
						drawerWithStockNeeded.put(drawerName,
								drawersWithChosenMedicine.get(drawerName) - (stock - stockNeeded));
						break;
					}
	...
	public void removeFromDrawer(Map<String, Integer> drawerWithStockNeeded, Medicine chosenMedicine) {
		...
		for (int i = 0; i < drawerNames.size(); i++) {
			...
			if (chosenMedicine instanceof EntireBoxMedicine) {
				...
				addToModificationsHistoryForRemoving(chosenMedicine, drawer.getName(),
						drawerWithStockNeeded.get(drawerNames.get(i)));
			}
		}
	}
	...
	public void addToModificationsHistoryForAdding(Medicine med, String id) {
		...
		if (med instanceof DivisibleMedicine) {
			modifications.add(new Modification(generateCurrentDate(), id, "+", med.getBrand(), med.getQuantity()));
		}
	...
	// Nu stiu exact daca e nevoie de metoda, dar la inceput aveam impresia ca
	// voi
	// avea nevoie de ea
	...
	// Aici ajung daca am introdus de 2 ori valori diferite fata de ceea ce
	// aveam
